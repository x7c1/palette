use crate::AppState;
use axum::{Json, extract::Query, extract::State, http::StatusCode};
use palette_domain::worker::WorkerId;
use std::sync::Arc;

use super::HookQuery;

pub async fn handle_session_start(
    State(state): State<Arc<AppState>>,
    Query(query): Query<HookQuery>,
    Json(payload): Json<serde_json::Value>,
) -> StatusCode {
    let worker_id_str = query.worker_id.as_deref().unwrap_or("unknown");
    let worker_id = match WorkerId::parse(worker_id_str) {
        Ok(id) => id,
        Err(e) => {
            tracing::warn!(worker_id = worker_id_str, error = ?e, "invalid worker_id in session-start hook");
            return StatusCode::BAD_REQUEST;
        }
    };

    tracing::info!(worker_id = worker_id_str, payload = %payload, "received session-start hook");

    super::save_session_id(state.interactor.data_store.as_ref(), &worker_id, &payload);

    StatusCode::OK
}
