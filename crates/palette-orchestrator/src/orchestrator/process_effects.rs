...
impl Orchestrator {
        ...
        while !pending.is_empty() {
            ...
            for effect in &pending {
                ...
                let (follow_up, new_job_effects) =
                    self.apply_task_effect(task_id, *new_status, &task_engine)?;
    ...
    /// Apply a single task status change.
    /// Returns (follow-up TaskEffects, RuleEffects from new Jobs).
    fn apply_task_effect(
        ...
        match new_status {
            TaskStatus::Ready => {
                let (task_effects, job_effects) = self.activate_ready_task(task_id, task_engine)?;
    ...
    fn activate_ready_task(
        ...
        if children.is_empty() {
            let job_effects = if let Some(task) = self.db.get_task(task_id)?
