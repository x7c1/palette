...
impl Database {
    pub fn list_jobs(&self, filter: &JobFilter) -> crate::Result<Vec<Job>> {
        ...
        let mut sql = "SELECT id, task_id, type_id, title, plan_path, description, assignee, status_id, priority, repository, pr_url, created_at, updated_at, notes, assigned_at FROM jobs WHERE 1=1".to_string();
