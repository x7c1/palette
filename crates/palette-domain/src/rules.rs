...
impl RuleEngine {
    ...
    pub fn on_status_change<S: TaskStore>(
        &self,
        store: &S,
        task_id: &TaskId,
        ...
        match (task.task_type, new_status) {
            ...
            (TaskType::Work, TaskStatus::InReview) => {
                let reviews = store.find_reviews_for_work(task_id)?;
                for review in reviews {
                    if review.status == TaskStatus::Todo || review.status == TaskStatus::Blocked {
                        store.update_task_status(&review.id, TaskStatus::Todo)?;
            ...
            (TaskType::Work, TaskStatus::Done) => {
                ...
                let assignable = store.find_assignable_tasks()?;
                for t in assignable {
                    effects.push(RuleEffect::AutoAssign {
                        task_id: t.id.clone(),
                    });
    ...
    pub fn on_review_submitted<S: TaskStore>(
        &self,
        store: &S,
        ...
        let mut effects = Vec::new();
        let work_tasks = store.find_works_for_review(review_task_id)?;
        ...
        match submission.verdict {
            Verdict::ChangesRequested => {
                ...
                if submission.round as u32 >= self.max_review_rounds {
                    for work in &work_tasks {
                        store.update_task_status(&work.id, TaskStatus::Escalated)?;
                ...
                for work in &work_tasks {
                    store.update_task_status(&work.id, TaskStatus::InProgress)?;
                    effects.push(RuleEffect::StatusChanged {
                        task_id: work.id.clone(),
            ...
            Verdict::Approved => {
                ...
                for work in &work_tasks {
                    let all_reviews = store.find_reviews_for_work(&work.id)?;
                    let all_approved = all_reviews.iter().all(|r| {
                        if r.id == *review_task_id {
                            return true; // This one is being approved now
                        }
                        let subs = store.get_review_submissions(&r.id).unwrap_or_default();
                        subs.last().is_some_and(|s| s.verdict == Verdict::Approved)
                    });
                    if all_approved {
                        store.update_task_status(&work.id, TaskStatus::Done)?;
                        effects.push(RuleEffect::StatusChanged {
                            task_id: work.id.clone(),
