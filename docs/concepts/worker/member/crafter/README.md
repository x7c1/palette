...
A Crafter is a [Member](../) that executes a Craft [Job](../../../job/). The Crafter produces deliverables — such as code changes, tests, or a [Blueprint](../../../blueprint/) (during the planning phase) — and is overseen by the [Approver](../../supervisor/approver/). When the Crafter needs to perform a potentially risky action, it sends a permission prompt to the Approver for approval.
...
- A Crafter implements a new feature and sends a permission prompt to the Approver before running a destructive command.
...
- request (permission from the Approver)

## Related Concepts

- [Member](../) — the Crafter is a kind of Member
- [Approver](../../supervisor/approver/) — the Supervisor that oversees the Crafter
