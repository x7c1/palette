...
A Crafter is a [Member](../) that executes a Craft [Job](../../../job/). The Crafter produces deliverables — such as code changes, tests, or a [Blueprint](../../../blueprint/) (during the planning phase) — and is overseen by the [Leader](../../supervisor/leader/). When the Crafter needs to perform a potentially risky action, it sends a permission prompt to the Leader for approval.
...
- A Crafter implements a new feature and sends a permission prompt to the Leader before running a destructive command.
...
- request (permission from the Leader)

## Related Concepts

- [Member](../) — the Crafter is a kind of Member
- [Leader](../../supervisor/leader/) — the Supervisor that oversees the Crafter
