...
1. **Supervised execution**: Instead of the Operator staying present to approve actions and make judgment calls, a Supervisor takes on that role. The [Leader](worker/supervisor/leader/) supervises a [Composite Task](task/) and its child Tasks, while the [Review Integrator](worker/supervisor/review-integrator/) consolidates findings from multiple [Reviewers](worker/member/reviewer/) into a single verdict.
