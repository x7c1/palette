...
A Composite Task is a Task that has child Tasks. A [Permission Supervisor](../worker/supervisor/permission-supervisor/) can be assigned to a Composite Task to handle permission prompts from [Members](../worker/member/) during execution and raise [Escalations](../escalation/) when a decision exceeds its confidence.
...
- [Permission Supervisor](../worker/supervisor/permission-supervisor/) — handles permission prompts for a Composite Task
