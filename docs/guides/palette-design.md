...
| palette-usecase | Use case layer (Interactor). Defines trait ports (ContainerRuntime, TerminalSession, DataStore, BlueprintReader) and the Interactor struct that mediates all external resource access. Also provides TaskStoreImpl (combines blueprint structure with execution state). Depends only on palette-domain. |
